...
p <- ggplot(data = data[outlier,], mapping = aes(x = sa_rank, y = group_effect)) +
  ...
  scale_y_continuous(limits = c(0.38, 0.8),breaks = seq(0.4, 0.8, by = 0.1)) +
...
ggsave(paste0(working_dir,'GNN_model/corr_hcp_pFC_eFC_group_effect_sa_rank.png'),plot=p,width = 10,height = 10,units = "cm",dpi = 600)
...
p <- ggplot(data = data[outlier,], mapping = aes(x = sa_rank, y = ind_effect)) +
  ...
  scale_color_gradient2(low = "#4393C3", high = "#D6604D", mid = "#F6FBFF",
                        midpoint = 200, limit = c(0,400)) + 
  scale_y_continuous(limits = c(-0.013, 0.04),breaks = seq(-0.01, 0.04, by = 0.01)) +
  scale_x_continuous(limits = c(0, 400),breaks = seq(0, 400, by = 100))
...
ggsave(paste0(working_dir,'GNN_model/corr_hcp_pFC_eFC_ind_effect_sa_rank.png'),plot=p,width = 10,height = 10,units = "cm",dpi = 600)
...
ggsave(paste0(working_dir,'GNN_model/corr_hcpd_pFC_eFC_group_effect_sa_rank.png'),plot=p,width = 10,height = 10,units = "cm",dpi = 600)
...
p <- ggplot(data = data[outlier,], mapping = aes(x = sa_rank, y = ind_effect)) +
  ...
  scale_color_gradient2(low = "#4393C3", high = "#D6604D", mid = "#F6FBFF",
                        midpoint = 200, limit = c(0,400)) + 
  scale_y_continuous(limits = c(-0.002, 0.05),breaks = seq(-0.0, 0.05, by = 0.01)) +
  scale_x_continuous(limits = c(0, 400),breaks = seq(0, 400, by = 100))
...
ggsave(paste0(working_dir,'GNN_model/corr_hcpd_pFC_eFC_ind_effect_sa_rank.png'),plot=p,width = 10,height = 10,units = "cm",dpi = 600)